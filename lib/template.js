...
function processMarkdown(markdown) {
  ...
  if (frontmatter) {
    html = renderFrontmatterHtml(frontmatter) + html;
  }
...
<body class="${!hasToc ? 'no-toc' : ''}">
  ...
  <header class="top-header">
    <div class="header-left">
      ...
      ${isLive ? '<span class="header-separator" aria-hidden="true">/</span><span class="doc-badge">LIVE</span>' : ''}
    </div>

    <div class="header-center">
      ${isLive ? '<span class="live-dot" title="Live sync active"></span>' : ''}
      <span class="doc-filename" id="app-doc-title">${filename}</span>
    </div>
  ...
  <script>
    ...
    function extractClientFrontmatter(md) {
      ...
      return { frontmatter: Object.keys(data).length > 0 ? data : null, body: body };
    }

    function renderClientFrontmatterHtml(data) {
      if (!data) return '';
      const entries = Object.entries(data);
      if (entries.length === 0) return '';
    ...
    function clientProcessMarkdown(markdown) {
      ...
      if (frontmatter) {
        html = renderClientFrontmatterHtml(frontmatter) + html;
      }
