  ...
  <script>
    ...
    function initDiagramPanZoom() {
      document.querySelectorAll('.mermaid-container').forEach(container => {
        ...
        // Extract native SVG viewBox coordinates
        let origVb = null;
        ...
        if (!origVb) {
          try {
            ...
            if (b && b.width > 0 && b.height > 0) {
              origVb = { x: b.x, y: b.y, w: b.width, h: b.height };
            }
          } catch (_) {}
        }
        if (!origVb) {
          const rect = svg.getBoundingClientRect();
          origVb = { x: 0, y: 0, w: Math.max(rect.width, 400), h: Math.max(rect.height, 200) };
        }

        // Set initial normalized viewBox and responsive sizing
        svg.setAttribute('viewBox', origVb.x + ' ' + origVb.y + ' ' + origVb.w + ' ' + origVb.h);
        svg.removeAttribute('height');
        svg.removeAttribute('width');
        svg.style.width = '100%';
        svg.style.maxWidth = Math.max(origVb.w, 480) + 'px';
        svg.style.height = 'auto';

        // Scaffold: container > .diagram-toolbar + .diagram-viewport > svg
        ...
        function zoomAtPoint(ratioX, ratioY, newScale) {
          ...
          const newW = origVb.w / clampedScale;
          const newH = origVb.h / clampedScale;
        ...
        // Wheel zoom (vector-sharp, centered at cursor position)
        ...
        // Pointer drag panning (1:1 smooth vector translation)
        let dragging = false;
