...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Fullscreen Mode */
    .mermaid-container.diagram-fullscreen {
    ...
    .mermaid-container.diagram-fullscreen .diagram-viewport {
      height: 100vh !important;
      max-height: none !important;
    }

    /* The interactive full-width viewport inside the container */
    .diagram-viewport {
      position: relative;
      display: flex;
      align-items: center;
      ...
      background-color: var(--bg-canvas);
      background-image: radial-gradient(var(--border-subtle) 1.2px, transparent 1.2px);
    ...
    /* Explicit Vector Styling: Overrides SVG default black fills on edges/markers */
    .mermaid-container svg .flowchart-link,
    .mermaid-container svg .edgePath .path,
    .mermaid-container svg path.flowchart-link,
    .mermaid-container svg [class*="flowchart-link"],
    .mermaid-container svg [class*="edgePath"] path {
      fill: none !important;
    ...
    .mermaid-container svg .arrowMarkerPath,
    ...
    .mermaid-container svg .node path:not(.flowchart-link):not(.path):not(.arrowMarkerPath) {
    ...
    /* Floating hint when scrolling without Ctrl */
    .diagram-hint {
      position: absolute;
      bottom: 0.85rem;
      left: 50%;
    ...
    .mermaid-container.diagram-fullscreen .diagram-toolbar {
      opacity: 1;
    }
...
<body class="${!hasToc ? 'no-toc' : ''}">
  ...
  <script>
    ...
    async function saveDocument() {
      ...
      const tauriInvoke = (tauri && tauri.core && typeof tauri.core.invoke === 'function' ? tauri.core.invoke : null)
        || (tauri && typeof tauri.invoke === 'function' ? tauri.invoke : null)
      ...
      if (tauriInvoke) {
        try {
          ...
          await tauriInvoke('save_document', { filePath: targetPath, markdown: currentMarkdown });
          setSaveStatus('saved');
          isDirty = false;
    ...
    // 7b. Lossless Native Vector Pan & Zoom for Mermaid diagrams
    function initDiagramPanZoom() {
      ...
      document.querySelectorAll('.mermaid-container').forEach(container => {
        ...
        if (!origDiag) {
          try {
            ...
            if (b && b.width > 0 && b.height > 0) {
              origDiag = { x: b.x, y: b.y, w: b.width, h: b.height };
            }
          } catch (_) {}
        }
        if (!origDiag) {
          const rect = svg.getBoundingClientRect();
          origDiag = { x: 0, y: 0, w: Math.max(rect.width, 400), h: Math.max(rect.height, 200) };
        }

        // Scaffold: container > .diagram-toolbar + .diagram-hint + .diagram-viewport > svg
        ...
        // Floating scroll hint (Google Maps / Figma UX)
        const hint = document.createElement('div');
        hint.className = 'diagram-hint';
        hint.textContent = (isMac ? '⌘ + Scroll to zoom' : 'Ctrl + Scroll to zoom') + ' • Drag to pan';
        ...
        btnReset.title = 'Reset view (Double-click)';
        btnReset.style.fontSize = '0.8rem';
        btnReset.textContent = '⟳';
        btnReset.onclick = () => container._pz.reset();

        const btnFullscreen = document.createElement('button');
        btnFullscreen.className = 'diagram-tool-btn';
        btnFullscreen.title = 'Toggle fullscreen';
        btnFullscreen.textContent = '⛶';
        btnFullscreen.onclick = () => container._pz.toggleFullscreen();
        ...
        // Aspect-matched viewBox calculations
        function calcBaseViewBox() {
          const padX = Math.max(origDiag.w * 0.05, 16);
          const padY = Math.max(origDiag.h * 0.05, 16);
          return {
            x: origDiag.x - padX,
            y: origDiag.y - padY,
            w: origDiag.w + padX * 2,
        ...
        let baseVb = calcBaseViewBox();
        let currentVb = { ...baseVb };
        let scale = 1.0;
        const MIN_SCALE = 0.2;
        const MAX_SCALE = 12.0;

        // Configure SVG for natural, undistorted vector scaling
        svg.removeAttribute('width');
        svg.removeAttribute('height');
        svg.style.maxWidth = 'none';
        svg.setAttribute('preserveAspectRatio', 'xMidYMid meet');

        // Explicitly sanitize all edge paths to prevent black SVG fill chords
        svg.querySelectorAll('path').forEach(p => {
          if (p.classList.contains('flowchart-link') || p.closest('.edgePath') || p.classList.contains('path')) {
            p.style.fill = 'none';
          }
        });
        ...
        function zoomAtPoint(ratioX, ratioY, newScale) {
          const clampedScale = Math.min(MAX_SCALE, Math.max(MIN_SCALE, newScale));
          if (Math.abs(clampedScale - scale) < 0.001) return;

          // Focal point in SVG coordinates
          const focalSvgX = currentVb.x + ratioX * currentVb.w;
          const focalSvgY = currentVb.y + ratioY * currentVb.h;

          // New viewBox dimensions
          const newW = baseVb.w / clampedScale;
          const newH = baseVb.h / clampedScale;

          // Preserve focal point under cursor
          const newX = focalSvgX - ratioX * newW;
          const newY = focalSvgY - ratioY * newH;

          currentVb = { x: newX, y: newY, w: newW, h: newH };
          scale = clampedScale;
          applyViewBox();
        }
        ...
        container._pz = {
          zoom(delta) {
            zoomAtPoint(0.5, 0.5, scale + delta);
          },
          reset() {
            scale = 1.0;
            baseVb = calcBaseViewBox();
            currentVb = { ...baseVb };
            applyViewBox();
          },
          toggleFullscreen() {
            const isFull = container.classList.toggle('diagram-fullscreen');
            btnFullscreen.textContent = isFull ? '✕' : '⛶';
            btnFullscreen.title = isFull ? 'Exit fullscreen (Esc)' : 'Toggle fullscreen';
        ...
        document.addEventListener('keydown', e => {
          if (e.key === 'Escape' && container.classList.contains('diagram-fullscreen')) {
            container._pz.toggleFullscreen();
          }
        });

        // Wheel zoom: prevent scrolljacking unless Ctrl/Cmd is pressed or in fullscreen
        let hintTimer = null;
        viewport.addEventListener('wheel', e => {
          const isCtrl = e.ctrlKey || (isMac && e.metaKey);
          const isFullscreen = container.classList.contains('diagram-fullscreen');

          if (isCtrl || isFullscreen) {
            e.preventDefault();
            const rect = svg.getBoundingClientRect();
            ...
            zoomAtPoint(ratioX, ratioY, scale * (1 + delta));
          } else {
        ...
        // Pointer drag panning (1:1 physical tracking across entire viewport)
        let dragging = false;
        ...
        viewport.addEventListener('pointerdown', e => {
          ...
          startVbX = currentVb.x;
          startVbY = currentVb.y;
        });
        ...
        viewport.addEventListener('pointermove', e => {
          ...
          const svgDeltaX = (dx / rect.width) * currentVb.w;
          const svgDeltaY = (dy / rect.height) * currentVb.h;
          currentVb.x = startVbX - svgDeltaX;
          currentVb.y = startVbY - svgDeltaY;
          applyViewBox();
        });
    ...
    function connectTauriBridge(retries) {
      if (typeof retries !== 'number') retries = 35;
      const tauri = window.__TAURI__;
      ...
      if (invoke) {
        invoke('get_initial_document')
          ...
          .catch(err => {
            console.error('Tauri initial load error:', err);
          });

        const listen = (tauri && tauri.event && typeof tauri.event.listen === 'function' ? tauri.event.listen : null)
          || (tauri && typeof tauri.listen === 'function' ? tauri.listen : null);

        if (listen) {
          listen('md-file-updated', (event) => {
            if (isDirty || isEditMode) return;
            const payload = event.payload;
            if (payload && typeof payload.markdown === 'string' && typeof renderMarkdownDocument === 'function') {
              renderMarkdownDocument(payload.markdown, payload.filename);
            }
          });
        }
      } else if (retries > 0) {
        setTimeout(() => connectTauriBridge(retries - 1), 40);
      }
    }

    connectTauriBridge();
    window.addEventListener('DOMContentLoaded', () => connectTauriBridge(20));
    window.addEventListener('load', () => connectTauriBridge(10));

    // 12. Check for initial document payload injected by native host
    function checkInitialRender() {
      if (window.__DOTMD_INITIAL_DATA__ && typeof window.__DOTMD_INITIAL_DATA__.markdown === 'string') {
        if (typeof renderMarkdownDocument === 'function') {
