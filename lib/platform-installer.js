...
function installWindows({ silent = false } = {}) {
  ...
  try {
    ...
    execSync(`reg add "${uninstallKey}" /v "DisplayName" /d "dot md" /f`, { stdio: 'ignore' });
    ...
    execSync(`reg add "${uninstallKey}" /v "UninstallString" /d "\\"${nodeExe}\\" \\"${cliPath}\\" --uninstall" /f`, { stdio: 'ignore' });
