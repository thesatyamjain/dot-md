...
function installWindows({ silent = false } = {}) {
  ...
  // 1. Check if dedicated installer/dotmd-setup.exe exists
  const dedicatedSetupExe = path.join(rootDir, 'installer', 'dotmd-setup.exe');
  if (fs.existsSync(dedicatedSetupExe)) {
    log(`  Found Windows installer: ${dedicatedSetupExe}`);
    const child = spawn(dedicatedSetupExe, ['/silent'], { stdio: 'ignore' });
  ...
  // 2. Check if a compiled Tauri installer exists
  ...
  // 3. Direct Windows Registry Integration (Zero compilation needed)
  try {
...
function uninstallWindows({ silent = false } = {}) {
  ...
  const dedicatedSetupExe = path.join(rootDir, 'installer', 'dotmd-setup.exe');
  if (fs.existsSync(dedicatedSetupExe)) {
    log(`  Running uninstaller: ${dedicatedSetupExe}`);
    const child = spawn(dedicatedSetupExe, ['/uninstall', '/silent'], { stdio: 'ignore' });
    return new Promise((resolve, reject) => {
      child.on('close', code => {
        if (code === 0) resolve({ ok: true });
        else reject(new Error(`Uninstaller exited with code ${code}`));
      });
    });
  }
