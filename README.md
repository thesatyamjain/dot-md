...
- **Desktop Application (Tauri)**:
  - Run `npm run build` (or `npx tauri build`) to compile standalone native `.msi` and `.exe` installers equipped with Start Menu integration, vector icon badges, and Windows Settings uninstaller entries.
  - Automated toolchain configuration: `powershell -ExecutionPolicy Bypass -File scripts/setup-tauri.ps1`.
- **CLI Shell Integration**:
  - Run `dotmd --install` (or `node bin/cli.js --install`) to configure Explorer right-click context actions (*"Preview with dot md"*), user `PATH` commands, and file associations for `.md`, `.markdown`, and `.mdown`.
  - **Uninstallation**: `dotmd --uninstall`.
